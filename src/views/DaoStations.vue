<template>
    <main id="main">
        <div class="content">
                ...
                <div class="container" style=" color:white; ">
                    <div class="card" v-for="stationClass in classes" v-bind:class="{'larger':stationClass.type}" style="min-height:600px;height:fit-content;align-items:center; padding: 5px 20px; text-align: center; min-width:200px; width:35%; overflow: auto;">
                        <div>
                            ...
                            <div v-if="selected === 'One Year'">Cost: {{stationClass.price/station1}} DAO</div>
                            <div v-if="selected === 'Two Years'">Cost: {{stationClass.price/station2}} DAO</div>
                            <div v-if="selected === 'Four Years'">Cost: {{stationClass.price/station4}} DAO</div>
            ...
            <div class="cards">
                <div class="container" style=" color:white; ">
                    <div class="card" v-for="stationClass in bridgeClasses" v-bind:class="{'larger':stationClass.type}" style="min-height:600px;height:fit-content;align-items:center; padding: 5px 20px; text-align: center; min-width:200px; width:35%; overflow: auto;">
                        <div>
                            <div style="font-size:x-large">{{stationClass.name}}</div>
                            <div v-if="stationClass.class === 'Meteor'" style="padding-top: 19%; padding-bottom: 19%; ">
                                <video controls width="100%" 
                            ...
                            <div v-if="selected === 'One Year'">Cost: {{stationClass.price/command1}} DAO</div>
                            <div v-if="selected === 'Two Years'">Cost: {{stationClass.price/command2}} DAO</div>
                            <div>Cost: {{stationClass.price}} Star</div>
                            <!-- <div>APR: {{stationClass.apr}}</div>
                            <div>Class: {{stationClass.class}}</div> -->
                            <div style="padding-top:10px; padding-bottom: 10px; text-align:left">{{stationClass.description}}</div>
    ...
    const bridgeClasses = require( "../components/CommandBridge.js");
    ...
    export default {
        ...
        data() {
            return {
                ...
                bridgeClasses:bridgeClasses.classes,
                station1:0.11,
                station2:0.12,
                station4:0.15,
                command1:0.17,
	...
	},
	{
		...
		"type": "function"
                ...
                daoContractAddress : "0xb7d68f17cE67a67CC2fE0ef0E1Bd6a48aF898683",
                daoContractInstance:null,
                daoControllerContractAbi : [
	{
		"inputs": [
			{
				"internalType": "uint256[]",
		...
		"outputs": [
			{
				...
				"internalType": "struct DaoStationLock.lockedCommand[]",
				"name": "",
				"type": "tuple[]"
			}
		],
		...
		"outputs": [
			{
				...
				"internalType": "struct DaoStationLock.lockedStation[]",
				"name": "",
				"type": "tuple[]"
			}
		],
	...
	},
	{
		...
		"name": "updateCommandControlAddress",
		"outputs": [],
		"stateMutability": "nonpayable",
		"type": "function"
	},
	...
	},
	{
		...
		"name": "updateStationControlAddress",
		"outputs": [],
		"stateMutability": "nonpayable",
		"type": "function"
	},
	{
		...
		"type": "function"
                ...
                daoControllerContractAddress : "0x38E42ca3cD1305d7aFe2dc23F692601AA7ba702e",
                daoControllerContractInstance: null,
            ...
            if (typeof window.ethereum !== 'undefined') {
                ...
                if(this.$route.params.web3 == null || this.$route.params.account == null){
                    ...
                    if(chainId != 0x89){await Functions.setChain('0x89')
                        this.$router.go();};
                    this.daoContractInstance = new this.web3.eth.Contract(this.daoContractAbi, this.daoContractAddress);
                    this.controllerContractInstance = new this.web3.eth.Contract(this.controllerContractAbi, this.controllerContractAddress);
                    this.daoControllerContractInstance = new this.web3.eth.Contract(this.daoControllerContractAbi, this.daoControllerContractAddress);
        ...
        methods: {
                    ...
                    if(result == 'Non-Ethereum browser detected. You should consider trying MetaMask!'){
                        ...
                        web3.eth.getAccounts().then((accounts) => {
                            if(accounts.length > 0){
                                ...
                                this.daoContractInstance = new web3.eth.Contract(this.daoContractAbi, this.daoContractAddress);
                                this.controllerContractInstance = new this.web3.eth.Contract(this.controllerContractAbi, this.controllerContractAddress);
                                this.daoControllerContractInstance = new web3.eth.Contract(this.daoControllerContractAbi, this.daoControllerContractAddress);
                                this.tokenContractInstance = new this.web3.eth.Contract(this.tokenURIABI, this.tokenContract);
            ...
            async mintStation(stationClass,lockLength){
                switch (lockLength){
                    case "One Year":
                        lockLength = 1;
                    case "Two Years":
                ...
                var allowance = await this.daoContractInstance.methods.allowance(this.account,this.daoControllerContractAddress).call();
                if(allowance < 1222*10**18){
                    var rates = await Functions.getRates();
                    try{
                        var receipt = await this.daoContractInstance.methods.approve(this.daoControllerContractAddress,ethers.utils.parseUnits("100000",18))
                            .send({
                            ...
                            if(receipt){
                                try{
                                    this.daoControllerContractInstance.methods.mintAndLockStation(stationClass,lockLength)
                                        .send({
                                            maxFeePerGas:(rates.fast.maxFee*(10**9)).toFixed(0),
                                            maxPriorityFeePerGas:(rates.fast.maxPriorityFee*(10**9)).toFixed(0),
                                            from:this.account
                ...
                if(allowance < 1222*10**18){
                    ...
                    try{
                        var receipt = await this.daoContractInstance.methods.approve(this.daoControllerContractAddress,ethers.utils.parseUnits("100000",18))
                            ...
                            if(receipt){
                                try{
                                    this.daoControllerContractInstance.methods.mintAndLockCommand(lockLength)
                                        .send({
                                            maxFeePerGas:(rates.fast.maxFee*(10**9)).toFixed(0),
                                            maxPriorityFeePerGas:(rates.fast.maxPriorityFee*(10**9)).toFixed(0),
                                            from:this.account
                                        }).then((res)=>{
                                            this.message = "Deploying Command Bridge"
                                            if(res){
                                                setTimeout(()=>{
                                                    this.controllerContractInstance.methods.getStations().call().then((newStations)=>{
                                                        console.log(newStations);
