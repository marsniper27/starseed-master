        ...
        <div class="content">
            ...
            <div class="cards">
                <div class="container" style=" color:white; ">
                    <div class="grid" style="width:100%; align-items: center; ">
                        <div class="card" style="min-width:98%;  align-items: center;">
                            <div class="grid">
                    ...
                    <div class="card" v-for="stationClass in classes" v-bind:class="{'larger':stationClass.type}" style="height:450px;align-items:center; padding: 5px 20px; text-align: center; width:35%; ">
                        <div>
                            <div>{{stationClass.name}}</div>
                            ...
                            <div>StartingWeight: {{stationClass.startingWeight}}</div>
                            <button style="left:0%; width:auto;" @click="mintStation(stationClass.pid)">Mint {{stationClass.name}}</button>
                        </div>
                    </div>
            </div>
        </div>
            <div class="cards" style=" color:white; ">
                <div class="sm-heading center">
                        Nexus Star Stations in Wallet
                </div>
                <div class="container">
                    <div class="card" v-for="station in starStations" v-bind:class="{'larger':station.type}" style="align-items:center">
                        ...
                        <button style="left:0%" @click="updateWeight(station)">Update Weight</button>
    ...
    export default {
        ...
        data() {
            return {
                ...
                tokenContract:"0x2AD4316391d004551c05bffb693040Df998d5143",//"0xEbF8F162A8EbB9Fe03BC94E6898B1d76843F8B22",//
                image:null
            }
        },
        watch: {
        ...
        methods: {
            ...
            async getNFTMetadata() {
                ...
                const numStations = await contract.methods.balanceOf(this.account).call();
                let stationInfo = await this.controllerContractInstance.methods.getStations().call();
                if(numStations >0){
                    for(let x = 0; x < numStations;x++){
                        let nftId = await contract.methods.tokenOfOwnerByIndex(this.account,x).call();
                        let nft = await contract.methods.tokenURI(nftId).call(); 
                        const url = nft.split("t/").pop();
                        const myFile = await fleekStorage.get({
                            apiKey: process.env.VUE_APP_apiKey,
                            apiSecret: process.env.VUE_APP_apiSecret,
                            key: url,
                            getOptions: [
                                'data'
                            ],
                        ...
                        text = text.substring(0, 1) + '"TokenId" : '+nftId +', "Class" : ' + stationInfo[nftId].class +','+ text.substring(1);
            ...
            async updateUri(station,newWeight){
                const classInfo = this.classes[station.Class];
                console.log("tokenId: "+ station.TokenId)
                const increase = ((newWeight/classInfo.startingWeight)-1)*100;
                var fileData = {
                    name :classInfo.name,
                    description : classInfo.description,
                    image : classInfo.image,
                ...
                const uploadedFile = await fleekStorage.streamUpload({
                    apiKey: process.env.VUE_APP_apiKey,
                    apiSecret: process.env.VUE_APP_apiSecret,
                    key: 'Star Stations/' +station.TokenId,
                    stream,
                });
                console.log(uploadedFile)
