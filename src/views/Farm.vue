...
export default {
    ...
    data() {
        return {
            ...
            starSeed: [{
                    ...
                    img: matic,
                    show: false,
                    address:"0xfbfe7c5c437effd9730913804e4cd8b890d62a9c",
    ...
    methods: {
        ...
        async StakeLP(itm){
            ...
            try{
                var receipt = await lpContractInstance.methods.approve(this.masterChefContractAddress,itm.amount).send({from: this.account})
                        console.log(receipt);
            }catch(error){
                console.log("approval: " +error);
            } try{
                 var receipt = await  this.masterChefContractInstance.methods.deposit(itm.pid,itm.amount).send({from: this.account})
                    console.log(receipt);
                    this.getUserPoolStats(itm);
            }catch(error){
                console.log("approval: " +error);
        ...
        async getUserPoolStats(itm){
                ...
                itm.balance = await this.getBalance(itm);
        ...
        async getBalance(itm){
            ...
            try{
                var receipt = await this.lpContractInstance.methods.balanceOf(this.account).call()
                    ...
                    return (receipt/10**18).toFixed(4);
            }catch(error){
